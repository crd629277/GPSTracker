...
public class GPSDatabase {
	...
	private static final String SQL_CREATE_LOCATION_TABLE = "create table "
			+ TABLE_NAME + " (" + "id integer primary key autoincrement, "
			+ "latitude double not null, " + "longitude double not null,"
			+ "speed double not null, " + "bearing float not null,"
			+ "altitude double not null," + "accuracy float not null,"
			+ "time long not null," + "del int default 0" + ");";

	private static final String SQL_CREATE_META_TABLE = "create table "
			+ META_TABLE_NAME + " (" + "id integer primary key autoincrement, "
			+ "name string not null unique," + "value string default null"
			+ ");";

	private SQLiteDatabase sqliteDatabase;

	public class Meta implements com.crd.gpstracker.dao.Meta {
		...
		public boolean addMeta(String tag, String value) {
			...
			try {
				return sqliteDatabase.insert(META_TABLE_NAME, null, values) > 0 ? true
						: false;
			} catch (SQLiteConstraintException e) {
				Log.e(TAG,
						"The name which marked as value: "
								+ tag
		...
		private String getMeta(String tag) {
			String result = "";
			try {
				String sql = "SELECT " + COLUMN.META_VALUE + " FROM "
						+ META_TABLE_NAME + " WHERE " + COLUMN.META_NAME
						+ " = '" + tag + "' LIMIT 1";
				...
				if (cursor.getCount() > 0) {
					result = cursor.getString(cursor
							.getColumnIndex(COLUMN.META_VALUE));
				}
				cursor.close();
		...
		try {
			cursor = sqliteDatabase.rawQuery(
					"SELECT count(id) AS count FROM " + TABLE_NAME
							+ " WHERE del = " + STATUS_NORMAL + " LIMIT 1",
					null);
			cursor.moveToFirst();
	...
	private Points getSingleLocationItemFromCursor(Cursor cursor) {
		...
		points.setLatitude(cursor.getDouble(cursor
				.getColumnIndex(COLUMN.LATITUDE)));
		points.setLongitude(cursor.getDouble(cursor
				.getColumnIndex(COLUMN.LONGITUDE)));
		points.setBearing(cursor.getFloat(cursor.getColumnIndex(COLUMN.BEARING)));
		points.setAltitude(cursor.getDouble(cursor
				.getColumnIndex(COLUMN.ALTITUDE)));
		points.setAccuracy(cursor.getFloat(cursor
				.getColumnIndex(COLUMN.ACCURACY)));
	...
	public Points getLastRecord() {
		...
		try {
			result = sqliteDatabase.rawQuery("SELECT *, count(*) as count "
					+ "FROM location WHERE del = 0 ORDER BY time DESC LIMIT 1",
					null);
			result.moveToFirst();

			if (result != null
					&& result.getLong(result
							.getColumnIndexOrThrow(COLUMN.COUNT)) > 0) {
			...
			Log.e(TAG,
					"Read SQLite Database Error, Maybe The Database NOT Ready?");
		}
			...
			cursor = sqliteDatabase
					.rawQuery(
							"SELECT DISTINCT latitude, longitude, speed, bearing, altitude, accuracy, time "
									+ " from location WHERE del = "
									+ STATUS_NORMAL + " ORDER BY time DESC",
							null);
