...
import com.crd.runtracker.activity.Preference;
...
import com.crd.runtracker.util.Notifier;
...
//    	protected int mCostTime = 0;
        ...
        @Override
        public void startRecord(int activityTypePosition) {
            if (getStatus() != Binder.STATUS_RECORDING) {
...
//	                        notifier.setCostTimeString(meta.getCostTimeStringByNow());
                ...
                editor.putInt(PREF_STATUS_FLAG, Binder.STATUS_RECORDING);
                editor.commit();
...
//        }
        ...
        public void resetStatus() {
        	SharedPreferences.Editor editor = sharedPreferences.edit();
        	editor.putInt(PREF_STATUS_FLAG, Binder.STATUS_STOPPED);
        	editor.commit();
        }
...
//                archive.close();
        ...
        @Override
        public int getStatus() {
            return sharedPreferences.getInt(PREF_STATUS_FLAG, Binder.STATUS_STOPPED);
        }
...
//        boolean autoStart = sharedPreferences.getBoolean(Preference.AUTO_START, false);
        boolean alreadyStarted = (serviceBinder.getStatus() == Binder.STATUS_RECORDING);
